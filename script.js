...
if(localStorage.length != null){
    ...
    document.getElementById('textarea').textContent = localStorage.getItem("text")
    }
}   
}
...
function normalize_text_entry(text_entry) {
    var normalised_text = text_entry.trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[.,\/#!$¿?'"%\^&\*;:{}=\-_`~()]/g,"").replace(/\s{2,}/g," ").replace(/\n/g, " ");
...
if(textarea_normalized.trim() && word_to_search_normalized.trim())
{
    var txt = "<span style='color:rgb(242, 102, 250);font-weight: bold;'>"+word_to_search_normalized.trim()+"</span>";
    if (getListe()[0] == null){
    ...
    else
    {
        ...
        resultHeader.textContent = "Le mot '"+word_to_search_normalized.trim()+"' apparait "+getListe().length+" fois dans le texte";
        ...
        for ( i in splitting_text){
            ...
            for (i in liste_information_text){   
                if (liste_three[liste_information_text[i]] == word_to_search_normalized.trim()){
